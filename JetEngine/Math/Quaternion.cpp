...
void Quaternion::FromRotationMatrix(const Matrix4& kRot)
{
	...
	//Matrix4 kRot = kRot2;//2.Transpose();
	//kRot.MakeTranspose();

	Real fTrace = kRot[0 * 4 + 0] + kRot[1 * 4 + 1] + kRot[2 * 4 + 2];
	Real fRoot;
	...
	if (fTrace > 0.0)
	{
		...
		x = (kRot[2 * 4 + 1] - kRot[1 * 4 + 2])*fRoot;
		y = (kRot[0 * 4 + 2] - kRot[2 * 4 + 0])*fRoot;
		z = (kRot[1 * 4 + 0] - kRot[0 * 4 + 1])*fRoot;
	}
	else
	{
		// |w| <= 1/2
		...
		size_t i = 0;
		if (kRot[1 * 4 + 1] > kRot[0 * 4 + 0])
			i = 1;
		if (kRot[2 * 4 + 2] > kRot[i * 4 + i])
			i = 2;
		size_t j = s_iNext[i];
		size_t k = s_iNext[j];

		fRoot = sqrt(kRot[i * 4 + i] - kRot[j * 4 + j] - kRot[k * 4 + k] + 1.0f);
		Real* apkQuat[3] = { &x, &y, &z };
		*apkQuat[i] = 0.5f*fRoot;
		fRoot = 0.5f / fRoot;
		w = (kRot[k * 4 + j] - kRot[j * 4 + k])*fRoot;
		*apkQuat[j] = (kRot[j * 4 + i] + kRot[i * 4 + j])*fRoot;
		*apkQuat[k] = (kRot[k * 4 + i] + kRot[i * 4 + k])*fRoot;
	}
}
//-----------------------------------------------------------------------
void Quaternion::ToRotationMatrix(Matrix4& kRot) const
{
	...
	kRot[0 * 4 + 0] = 1.0f - (fTyy + fTzz);
	kRot[0 * 4 + 1] = fTxy - fTwz;
	kRot[0 * 4 + 2] = fTxz + fTwy;
	kRot[1 * 4 + 0] = fTxy + fTwz;
	kRot[1 * 4 + 1] = 1.0f - (fTxx + fTzz);
	kRot[1 * 4 + 2] = fTyz - fTwx;
	kRot[2 * 4 + 0] = fTxz - fTwy;
	kRot[2 * 4 + 1] = fTyz + fTwx;
	kRot[2 * 4 + 2] = 1.0f - (fTxx + fTyy);
}
//-----------------------------------------------------------------------
...
void Quaternion::FromAxes(const Vector3* akAxis)
{
	...
	{
		kRot[0 * 4 + iCol] = akAxis[iCol].x;
		kRot[1 * 4 + iCol] = akAxis[iCol].y;
		kRot[2 * 4 + iCol] = akAxis[iCol].z;
	}
...
void Quaternion::ToAxes(Vector3* akAxis) const
{
	...
	for (size_t iCol = 0; iCol < 3; iCol++)
	{
		akAxis[iCol].x = kRot[0 * 4 + iCol];
		akAxis[iCol].y = kRot[1 * 4 + iCol];
		akAxis[iCol].z = kRot[2 * 4 + iCol];
	}
}
//-----------------------------------------------------------------------
Vector3 Quaternion::xAxis(void) const
{
	...
	return Vector3(1.0f - (fTyy + fTzz), fTxy - fTwz, fTxz + fTwy);//Vector3(1.0f-(fTyy+fTzz), fTxy+fTwz, fTxz-fTwy);
}
//-----------------------------------------------------------------------
	...
	const Quaternion& rkP, const Quaternion& rkQ, int iExtraSpins)
{
	...
	Real fSin = sin(fAngle);
	Radian fPhase(3.1415926535895f*iExtraSpins*fT);
	Real fInvSin = 1.0f / fSin;
